...
(function(ext) {
	...
	ext.getTimer = function(nextID){
		...
		_device.responseValue(nextID,new Date().getTime()-startTimer);
	}
	ext.mapNumber = function(nextID,num,oldMin,oldMax,newMin,newMax){
		_device.responseValue(nextID,(num-oldMin)/(oldMax-oldMin)*(newMax-newMin)+newMin);
	}
	ext.constrainNumber = function(nextID,num,min,max){
		_device.responseValue(nextID, num<min?min:num>max?max:num);
	}
    ...
    function tryNextDevice() {
        // If potentialDevices is empty, _device will be undefined.
        // That will get us back here next time a _device is connected.
    ...
    function deviceOpened(dev) {
       ...
       //     tryNextDevice();
        ...
        //_device.set_receive_handler('arduino',processData);
    };
    ...
    ext._getStatus = function() {
        if(!_device) return {status: 1, msg: 'Arduino disconnected'};
        if(watchdog) return {status: 1, msg: 'Probing for Arduino'};
        return {status: 2, msg: 'Arduino connected'};
    }

    var descriptor = {};
	ScratchExtensions.makeblockRegister('Arduino', descriptor, ext, {type: 'serial'});
})({});
