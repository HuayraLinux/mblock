...
var ueditor = function (static_url, temp_url, handel) {
  return function (req, res, next) {
    var _respond = respond(static_url, temp_url, handel);
    _respond(req, res, next);
  };
};
var respond = function (static_url, temp_url, callback) {
  return function (req, res, next) {
    req.body = {};
    if (req.query.action === 'config') {
      ...
      busboy.on('file', function (fieldname, file, filename, encoding, mimetype) {
        ...
        res.ue_up = function (img_url) {
          ...
          var temp_dest = path.join(temp_url, img_url, name);

          file.pipe(fs.createWriteStream(tmpdir));
          fse.move(tmpdir, dest, function (err) {
            if (err) throw err;
            fse.copy(dest, temp_dest, function (err) {
              if (err) throw err;
              res.json({
