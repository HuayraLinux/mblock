...
bluetoothSerialPort.on('found', function(address, name) { // 已找到蓝牙设备
	...
	var regex="\\((.+?)\\)",tmpAddress=address;
	if(address.indexOf('(') > -1 && address.indexOf(')') > -1 ){
		var foundAddress=address.match(regex);
		tmpAddress =foundAddress[1];
	}

	console.log('tmpAddress:',tmpAddress);
	bluetoothSerialPort.findSerialPortChannel(tmpAddress, function(channel) { // 找到多少个蓝牙，就是循环多少次
		name = name + "(" + tmpAddress + ")";
		// 找到一个通知主进程
		var device = {
			'label'   : name,
			'address' : tmpAddress,
		...
		process.send({
			'method': 'foundBluetooth',
			'device': device
		});
		console.log('已找到一个蓝牙[foundBluetooth]：', device);
